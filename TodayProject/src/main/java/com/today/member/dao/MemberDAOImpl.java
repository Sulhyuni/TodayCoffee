...
public class MemberDAOImpl implements MemberDAO{
	...
	public MemberVO login(MemberVO inVO) throws Exception {
		...
		try {
			...
			String sql = "select m.id, m.name, m.gradeNo, g.gradeName, m.status, m.point "
...
//				if()
	...
	public String findId(MemberVO vo) throws Exception {
		...
		try {
			...
			if(rs.next()&&rs!=null) {
				id = rs.getString("id");
			}else throw new Exception("입력하신 정보의 id를 찾을 수 없습니다.");
		} catch (Exception e) {
			// TODO: handle exception
			e.printStackTrace();
			throw new Exception("입력하신 정보의 id를 찾을 수 없습니다.");
		}finally {
			DB.close(con, pstmt, rs);
		}
		return id;
	...
	public String findPw(MemberVO vo) throws Exception {
		...
		try {
			...
			// TODO: handle exception
			e.printStackTrace();
			throw new Exception("입력하신 정보의 비밀번호를 찾을 수 없습니다.");
		}finally {
			DB.close(con, pstmt, rs);
		}
		return pw;
	...
	public MemberVO checkPw(MemberVO vo) throws Exception {
		...
		try {
			...
			// TODO: handle exception
			e.printStackTrace(); // 개발자를 위한 코드
			throw new Exception("비밀번호가 틀렸습니다. 다시 입력해주세요");
		} finally {
			// 7. 닫기
			DB.close(con, pstmt, rs);
		} // end of try ~ catch ~ finally
	...
	public int changeGradeNo(MemberVO vo) throws Exception {
		...
		try {
			...
			// TODO: handle exception
			e.printStackTrace();
			throw new Exception("등급 수정 DB처리중 오류 발생");
		}finally {
			DB.close(con, pstmt);
		}
		return result;
	...
	public int changePwNow(MemberVO vo) throws Exception {
		...
		try {
			...
			// TODO: handle exception
			e.printStackTrace();
			throw new Exception("비밀번호즉시변경 DB처리중 오류 발생");
		}finally {
			DB.close(con, pstmt);
		}
		return result;
	...
	public int wakeUp(MemberVO vo) throws Exception {
		// TODO Auto-generated method stub
		System.out.println("MemberDAOImpl.wakeUp().vo : " + vo);
		int result = 0;
		try {
			con = DB.getConnection();
			String sql = "update member set status='정상' where id = ? and pw = ? and name = ? and tel1 = ? and tel2 = ? and tel3 = ?";
			System.out.println("sql : " + sql);
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, vo.getId());
			pstmt.setString(2, vo.getPw());
			...
			result = pstmt.executeUpdate();
//			System.out.println(result);
			if(result==0) throw new Exception("휴면해제 DB 처리중 오류 발생");
		} catch (Exception e) {
			// TODO: handle exception
			e.printStackTrace();
			throw new Exception("입력하신 정보의 데이터가 없습니다. 다시 입력해주세요");
		}finally {
			DB.close(con, pstmt);
		}
		return result;
	...
	public int sleep(Date date) throws Exception {
		// TODO Auto-generated method stub
		System.out.println("MemberDAOImpl.sleep().date : " + date);
		int result = 0;
		try {
			con = DB.getConnection();
			...
			// TODO: handle exception
			e.printStackTrace();
			throw new Exception("휴면처리 DB처리중 오류 발생");
		}finally {
			DB.close(con, pstmt);
		}
		return result;
