...
public class OrderDAOImpl implements OrderDAO{
	...
	public List<OrderVO> list(PageObject pageObject) throws Exception {
		...
		try {
			...
			e.printStackTrace();
			throw new Exception("주문내역 리스트 DB처리 중 오류 발생");
		} finally {
			DB.close(con, pstmt, rs);
		}
		return list;
	...
	public List<OrderVO> perList(PageObject pageObject, String id) throws Exception {
		System.out.println("OrderDAOImpl.perList()");
		List<OrderVO> list = null;
		try {
			con = DB.getConnection();
			// 주문번호, 아이디, 주문일자, 결제금액, 주문상태
			...
			e.printStackTrace();
			throw new Exception("개인주문내역 리스트 DB처리 중 오류 발생");
		} finally {
			DB.close(con, pstmt, rs);
		}
		return list;
	...
	public OrderVO view(long no) throws Exception {
		...
		try {
			...
			String sql = " SELECT od.orderNo, m.id, g.fileName, g.name_kr, g.price, g.goodsDiv, ot.payMethod, "
	...
	public int delete(OrderVO vo) throws Exception {
		...
		try {
			...
			System.out.println("sql : " + sql);
			pstmt = con.prepareStatement(sql);
			pstmt.setLong(1, vo.getUsePoint());
	...
	public List<OrderVO> mmList(List<Long> noList) throws Exception {
		System.out.println("OrderDAOImpl.mmList()");
		List<OrderVO> list = null;
		try {
			con = DB.getConnection();
			// 주문번호, 아이디, 주문일자, 결제금액, 주문상태
			String sql = " SELECT u.mmNo, m.id, g.goodsNo, g.name_kr, g.goodsDiv, g.fileName, "
					+ " o.optionNo, o.cup_size, o.cup, o.syrup, o.heating, u.totalPrice "
					+ " FROM Mymenu u, goods g, goods_option o, member m "
					+ " WHERE u.id = m.id AND u.goodsNo = g.goodsNo AND u.optionNo = o.optionNo"
					+ " AND u.mmNo IN ( ";
			for (int i = 0; i < noList.size(); i++) {
				if(i > 0) {
					sql += ",";
	...
	public int cartDelete(List<Long> noList) throws Exception {
		System.out.println("OrderDAOImpl.cartDelete()");
		int result = 0;
		try {
			con = DB.getConnection();
	...
	public int stockUp(OrderVO vo) throws Exception {
		int result = 0;
		try {
			con = DB.getConnection();
			String sql = " UPDATE goods SET stock = (stock - ?)"
					+ " WHERE goodsNo = ? ";
			pstmt = con.prepareStatement(sql);
			pstmt.setLong(1, vo.getStockQuantity());
