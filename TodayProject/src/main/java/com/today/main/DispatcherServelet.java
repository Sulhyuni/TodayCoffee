...
public class DispatcherServelet extends HttpServlet {
	...
	public void destroy() {
		...
		System.out.println("DispatcherServelet.destroy() - 쓰레드 mst 종료");
		Init.mst.interrupt();
		Init.mst.stop();
	}
	...
	public void init(ServletConfig config) throws ServletException {
		...
		// orcle 드라이버 확인
		try {
			Class.forName("com.today.io.db.DB");
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (uri.equals("/") || uri.equals("/index.do")) {
			System.out.println((MainController)Init.getController("mainController"));
			jsp = ((MainController)Init.getController("mainController")).execute(request);
		} else if(uri.indexOf("/order/") == 0) {
			// OrderController 실행
			System.out.println((OrderController)Init.getController("orderController"));
			jsp = ((OrderController)Init.getController("orderController")).execute(request);
		} else if(uri.indexOf("/member/") == 0) {
			// MemberController 실행
			System.out.println((MemberController)Init.getController("memberController"));
			jsp = ((MemberController)Init.getController("memberController")).execute(request);
		} else if(uri.indexOf("/goods/") == 0) {
			// GoodsController 실행
			jsp = ((GoodsController)Init.getController("goodsController")).execute(request);
			...
			// CartController 실행
			System.out.println((CartController)Init.getController("cartController"));
			jsp = ((CartController)Init.getController("cartController")).execute(request);
		} else if(uri.indexOf("/mymenu/") == 0) {
			// MyMenuController 실행
			System.out.println((MyMenuController)Init.getController("mymenuController"));
			jsp = ((MyMenuController)Init.getController("mymenuController")).execute(request);
		} else if(uri.indexOf("/event/") == 0) {
			// eventController 실행
			System.out.println((EventController)Init.getController("eventController"));
			jsp = ((EventController)Init.getController("eventController")).execute(request);
