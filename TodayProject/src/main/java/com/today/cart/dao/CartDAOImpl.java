...
//			pstmt.setString(1, id);
	...
	public int update(CartVO vo) throws Exception{
		...
		try {
			...
			con = DB.getConnection();
			// 3. sql
			String sql = " update cart set quantity = ?, totalPrice = ? where cartNo = ?";
			// 4. 실행객체
			pstmt = con.prepareStatement(sql);
			pstmt.setLong(1, vo.getQuantity());
			pstmt.setLong(2, vo.getTotalPrice());
			pstmt.setLong(3, vo.getCartNo());
			// 5. 실행
			result = pstmt.executeUpdate();
			// 6. 데이터 저장
		} catch (Exception e) {
...
//			pstmt.setLong(6, vo.getQuantity());
	...
	// optionNo 가져오는 메서드
		...
		public CartVO mmNo(long mmNo)throws Exception{
			CartVO mmNoVO = null;
			try {
				// 1. 확인 2. 연결con = DB.getConnection();
				con = DB.getConnection();
				// 3. sql 
				String sql = " SELECT goodsNo, optionNo FROM mymenu WHERE mmNo = ? ";
					// 4. 실행객체
					pstmt = con.prepareStatement(sql);
					pstmt.setLong(1, mmNo);
				// 5. 실행
				rs = pstmt.executeQuery();
				if(rs != null && rs.next()) {
					mmNoVO = new CartVO();
					mmNoVO.setGoodsNo(rs.getLong("goodsNo"));
					mmNoVO.setOptionNo(rs.getLong("optionNo"));
				}
				// 6. 데이터 저장
			} catch (Exception e) {
				e.printStackTrace();
