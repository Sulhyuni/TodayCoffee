...
public class CartController {
	...
	private ServiceInterface cartNoListService;
	private ServiceInterface cartmmNoService;
	// setter
	...
	public void setCartNoListService(ServiceInterface cartNoListService) {
		this.cartNoListService = cartNoListService;
	}
	public void setCartmmNoService(ServiceInterface cartmmNoService) {
		this.cartmmNoService = cartmmNoService;
	}
	...
	public String execute(HttpServletRequest request) throws Exception {
		...
		case "/cart/add.do": // 담기
				...
				String mmNoStr = request.getParameter("mmNo");// mmNo를 변수에 저장
				System.out.println(mmNoStr);
				if(mmNoStr != null) { // mmNoStr이 null이 아닐경우
					long mmNo = Long.parseLong(mmNoStr);
					vo = (CartVO) Execute.run(cartmmNoService, mmNo);
					vo.setQuantity(1);
					vo.setTotalPrice(Long.parseLong(request.getParameter("totalPrice")));
				} else {	// mmNoStr이 null인 경우
