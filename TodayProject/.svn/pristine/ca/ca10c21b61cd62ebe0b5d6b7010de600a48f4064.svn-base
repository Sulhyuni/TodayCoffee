...
public class MyMenuDAOImpl implements MyMenuDAO{
	...
	public List<MyMenuVO> myMenuList(List<Long>noList)throws Exception{
		...
		try {
			...
			con = DB.getConnection();
			// 3. sql
			String sql = " SELECT mm.cartNo, m.id, g.goodsNo, g.name_kr, g.goodsDiv, g.fileName, "
					+ " o.optionNo, o.cup_size, o.cup, o.syrup, o.heating, mm.mmName, mm.totalPrice "
					+ " FROM mymenu mm, goods g, goods_option o, member m "
					+ " WHERE mm.id = m.id AND mm.goodsNo = g.goodsNo AND mm.optionNo = o.optionNo"
					+ " AND mm.mmNo IN ( ";
