...
////지난 이벤트
	...
	public Integer update(EventVO vo) throws Exception {
		...
		try {
			...
			System.out.println("1,2.확인,연결");
			//3.sql
			String sql = "update event set title=?, content=?, startDate=?, endDate=?, updateDate=sysdate where eventNo = ?  ";
	...
	//응모현황
	@Override
	public List<EventVO> applyList() throws Exception {
		...
		@Override
		public List<EventVO> userApplyList(String id) throws Exception {
			System.out.println("EventDAOImpl.userApplyList()...");
			List<EventVO> list = null;
			...
			try {
				...
				String sql = "select w.winNo, e.eventNo, e.title, m.id, w.win "
						+ " from win w, event e, member m "
						+ " where (w.id = m.id) and (w.eventNo = e.eventNo) and (m.id = ?) "
						+ " order by winNo desc";
				System.out.println("3. sql :" + sql);
				// 4 실행객체생성 및 데이터 세팅
				pstmt = con.prepareStatement(sql);
